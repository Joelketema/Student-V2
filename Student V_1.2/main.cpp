...
char functionMenu()
{
	...
	cout << "\t"<< char(186) << "   \t\t 7.To Restore All Deleted Records     \t     " << char(186) << endl;
	cout << "\t"<< char(186) << "   \t\t 8.To Change login information     \t     " << char(186) << endl;
	cout << "\t"<< char(186) << "   \t\t 9.TO EXIT\t\t\t             " << char(186) << endl;
	cout << "\t"<< char(186) << "   \t\t 10.MAIN MENU\t\t\t             " << char(186) << endl;
	cout << "\t"<< char(186) << "                    \t\t\t\t\t     " << char(186)<<endl;
	cout << "\t"<< "   \t\t\t ->Input Here: ";
	choice = validation();
    //validate(choice);
...
void getinfo()
{
    ...
    cout << "\t\t" << char(192);      for(int i=0; i<39; i++){cout << char(196);}      cout << char(217) << endl;
    cout<<endl;
    cout << "\t\t"<< "  Enter Student ID:  ";s.id_no = validation();
    cout<<endl;
    ...
    cout<<"\t\t\t" << endl;
        ...
        for(int j=0;j<s.course_number; j++ )
        {
            ...
            cout<<"\t\t\t"<< endl;
            cout << "\t\t" << "  Enter credit Hour: ";s.c[j].credit_hour = validation();
            cout<<"\t\t\t" << endl;
            cout << "\t\t" <<"  Enter the assessment result: ";cin>> s.c[j].assesment; cout<<endl;
            int V = validate(s.c[j].assesment);
            cout<<"\n";
            if(V==1)
            {
                  cout << "\t\t" << "  Enter the Final Result: ";cin>>s.c[j].finalResult;
                  int X = validate(s.c[j].finalResult);
                  if(X==1)
                  {
                    cout<<"\n";
                    cout<<"\t\t\t" << endl;
...
int restore()
{
	...
	rename("backup.dat", "student.txt");
	rename("cback.dat", "courses.txt");
...
void createBackup()
{
	ofstream bfile("backup.dat", ios::app);
	ofstream cbfile("Cback.dat", ios::app);
	bfile << s.id_no;
	bfile << s.studentname <<endl;
	bfile << s.section << " "<< s.sex << " " << s.course_number << endl;
	bfile << s.gpa <<" "<< s.rankk << endl;

	for(int i = 0; i < s.course_number ;i++)
    {
        cbfile << s.id_no;
        cbfile << s.c[i].course_name << endl;
        cbfile << s.c[i].credit_hour << endl;
        cbfile << s.c[i].grade << " " << s.c[i].passorfail << endl;
    }
}
/***change admin info***/
int changeAdminInfo()
